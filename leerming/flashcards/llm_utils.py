...
def make_flashcards_from(source_text:str, main_focus_point:str, max_result:int=10):
    ...
    for statement in result:
        ...
        for cz in clozed_deletions:
            flashcards.append(UnsavedFillInTheGapCard(statement, cz))
    return flashcards




def save_tmp_flashcard_to_session(request:HttpRequest, flashcards:list[UnsavedFillInTheGapCard]):
...
def load_tmp_flashcard_from_session(request:HttpRequest):
    try:
        return [UnsavedFillInTheGapCard(**flashcard) for flashcard in request.session["tmp_flashcards"]]
    except KeyError:
        return []
